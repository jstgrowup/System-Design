...
export interface AcquireSeatLocksResult {
  acquired: boolean;
 ...
 * transaction with FOR UPDATE NOWAIT (in inventory-service) catches those
 * conflicts.
 */
 ...
 * @param bookingId Used as lock value for ownership
...
export async function acquireSeatLocks(
  ...
  fromSeq?: number,
  toSeq?: number,
): Promise<AcquireSeatLocksResult> {
 ...
 * @param lockValue The lock value returned from acquireSeatLocks
 */
 ...
 */
export async function forceReleaseSeatLocks(
  scheduleId: string,
  seatIds: string[],
  fromSeq?: number | null,
  toSeq?: number | null,
): Promise<void> {
  const keys = buildLockKeys(
    scheduleId,
    seatIds,
    fromSeq ?? undefined,
    toSeq ?? undefined,
  ...
  try {
    ...
    logger.error("Error force-releasing locks", {
      error: (error as Error).message,
    });
  }
}
