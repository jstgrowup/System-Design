...
export interface BookingFailedData {
  bookingId: string;
  firstName?: string;
  reason: FailureReason;
}

export type CancellationReason = "user_cancelled" | "schedule_cancelled";
...
export function getOtpTemplate(otp: string, ttlMinutes: number): string {
  return `
    <div style="
...
export function getWelcomeTemplate(firstName: string): string {
  return `
    <div style="
      font-family: Arial, sans-serif; 
      max-width: 420px; 
    ...
    ">
      ...
      <div style="text-align: center; margin: 25px 0;">   
        <a href="${config.ALLOWED_ORIGINS}/login" 
          style="
            display: inline-block;
            padding: 12px 22px;
            background: #4A3AFF;
...
const CANCELLATION_REASON_MESSAGES: Record<CancellationReason, string> = {
  user_cancelled: "You requested to cancel this booking.",
  schedule_cancelled:
    "The train schedule for this booking was cancelled by IRCTC.",
};

export function getBookingConfirmedTemplate(
...
export function getBookingFailedTemplate(data: BookingFailedData): string {
