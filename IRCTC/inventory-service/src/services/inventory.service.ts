...
const initializeInventory = async (
  eventData: ScheduleCreatedEventData,
): Promise<void> => {
  ...
  await prisma.$transaction(async (tx: TransactionClient) => {
    ...
    // Persist route topology for segment overlap checks.
...
const cancelScheduleInventory = async (
  eventData: ScheduleCancelledEventData,
): Promise<void> => {
  ...
  try {
    ...
    logger.error(
      ...
      { scheduleId, error: (err as Error).message },
    );
  }
};
...
async function recomputeSegmentSeatStatuses(
  ...
  for (const seatId of seatIds) {
    ...
    const current = await tx.$queryRaw<SeatStatusRow[]>`
    ...
    if (oldStatus === "AVAILABLE" && newStatus !== "AVAILABLE")
      statusChanges.nowOccupied++;
    if (oldStatus !== "AVAILABLE" && newStatus === "AVAILABLE")
      statusChanges.nowAvailable++;
    if (oldStatus === "LOCKED" && newStatus === "BOOKED")
      statusChanges.lockedToBooked++;
    if (oldStatus === "BOOKED" && newStatus === "LOCKED")
      statusChanges.bookedToLocked++;
...
const getSeats = async (
  ...
  const where: { scheduleId: string; status?: SeatStatus; seatType?: string } =
    { scheduleId };
  if (filters.status) where.status = filters.status;
  if (filters.seatType) where.seatType = filters.seatType;

  let seats: SeatSummaryWithSegment[] = await prisma.seatInventory.findMany({
    where,
  ...
  // If segment specified, compute per-seat segment availability.
  if (filters.fromSeq && filters.toSeq) {
    const fromSeq = parseInt(String(filters.fromSeq), 10);
    const toSeq = parseInt(String(filters.toSeq), 10);
    ...
    const blockedSeatIds = new Set(
      overlappingLocks.map((l: SeatIdRow) => l.seatId),
    );

    // Find seats that have ANY segment locks (to distinguish legacy bookings)
    const seatsWithAnyLock = await prisma.seatSegmentLock.findMany({
      where: { scheduleId, status: { in: ["LOCKED", "BOOKED"] } },
      select: { seatId: true },
      distinct: ["seatId"],
    });
    const seatsWithLocks = new Set(
      seatsWithAnyLock.map((l: SeatIdRow) => l.seatId),
    );
...
const lockSeats = async (
  ...
  const ttl = Math.min(
    Math.max(ttlSeconds || config.LOCK_TTL_SECONDS, 60),
    600,
  );
  ...
  const result = await retryTransaction(async () => {
    return prisma.$transaction(
      async (tx: TransactionClient) => {
        ...
        const seats = await tx.$queryRaw<SeatInventoryRow[]>`
        ...
        // Check segment-level availability instead of full-journey.
        if (fromSeq && toSeq) {
          // Check for overlapping segment locks on any of the requested seats
          const overlapping = await tx.$queryRaw<SeatIdRow[]>`
  ...
  try {
    ...
    logger.error("Failed to publish availability after lock", {
  ...
  const result = await retryTransaction(async () => {
    return prisma.$transaction(
      async (tx: TransactionClient) => {
        ...
        // Confirm segment locks if segment params provided.
        if (fromSeq && toSeq) {
      ...
      async (tx: TransactionClient) => {
        ...
        if (segmentLocks.length > 0) {
          ...
          const affectedSeatIds = [
            ...new Set(segmentLocks.map((l: { seatId: string }) => l.seatId)),
          ];
          await recomputeSegmentSeatStatuses(tx, scheduleId, affectedSeatIds);
          const counts = await recountScheduleAggregates(tx, scheduleId);
...
const recountAndPublish = async (
  ...
  try {
    ...
    logger.error("Failed to publish availability after recount", {
      scheduleId,
      error: (err as Error).message,
    });
  }
