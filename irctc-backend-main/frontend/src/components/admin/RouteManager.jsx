...
export default function RouteManager() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!selectedTrain) {
      showToast("Select a train", "warning");
      return;
    }
    setCreating(true);
    try {
    ...
    <form onSubmit={handleCreate} className="card">
      ...
      <div className="space-y-3 mb-4">
        {stops.map((stop, i) => (
          <div
            key={i}
            className="flex flex-wrap gap-2 items-end bg-gray-50 rounded-lg p-3"
          >
            <div className="w-10 text-center text-sm font-bold text-gray-400">
              {i + 1}
            </div>
            ...
            <Input
              ...
              onChange={(e) =>
                updateStop(i, "distanceFromOrigin", e.target.value)
              }
              className="w-28"
            />
            {stops.length > 1 && (
              <button
                ...
                className="text-red-500 hover:text-red-700 text-xl pb-1"
              >
                &times;
              </button>
            )}
